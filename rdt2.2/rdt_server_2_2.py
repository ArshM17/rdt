...
while True:
    try:
        ...
        if cal_checksum(message) != checksum:
            print(f"Received corrupted message. Sending previous ACK.{expected_seq-1%2}")
            checksum = cal_checksum("ACK")
            if random.random() > LOSS_PROBABILITY:
                checksum = cal_checksum("ERROR")
            packet = f"ACK:{checksum}:{(expected_seq-1)%2}"
            server_socket.sendto(packet.encode(), client_address)
        elif int(seq_no) != expected_seq % 2:
            print(f"Incorrect Sequence Number. Sending previous ACK.{expected_seq-1%2}")
            checksum = cal_checksum("ACK")
            if random.random() > LOSS_PROBABILITY:
                checksum = cal_checksum("ERROR")
            packet = f"ACK:{checksum}:{(expected_seq-1)%2}"
            ...
            print(f"Received message: \"{message}\". Sending ACK.{expected_seq%2}")
            checksum = cal_checksum("ACK")
